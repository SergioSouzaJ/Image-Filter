...
#define NUM_PASOS 5
#define DIST_RADIO 0
#define IMAGEN_ENTRADA "pug.ppm"
#define IMAGEN_SALIDA "pugResult.ppm"
...
int lee_ppm(char *nomfich, struct pixel ***img, int *nf, int *nc)
{
  ...
  fscanf(df, "%s", tipo);
  if (strcmp(tipo, "P3") != 0) {
    printf("Formato erróneo. Se esperaba tipo P3\n");
    return -1;
  }
  ...
  if ((*img = (struct pixel **)malloc(sizeof(struct pixel *) * height)) == NULL) {
    printf("Error reservando memoria para %d filas\n", height);
    return -1;
  }
  if (((*img)[0] = (struct pixel *)malloc(sizeof(struct pixel) * width * height)) == NULL) {
    printf("Error reservando memoria para %d * %d elementos\n", width, height);
    return -1;
  }
  for (i = 1; i < height; i++)
    (*img)[i] = (*img)[i - 1] + width;
           ...
           int n, int m)
{
  ...
  if ((ppdBloque = (int **)malloc(sizeof(int *) * (2 * (radio + 1)))) == NULL) {
    printf("Error reservando memoria para el bloque del filtrado\n");
    exit(-1);
  }
      ...
      for (j = 0; j < m; j++) {
        ...
        for (k = max(0, i - radio); k <= min(n - 1, i + radio); k++) {
          for (l = max(0, j - radio); l <= min(m - 1, j + radio); l++) {
            ...
            resultado.r += min(max(ppsImagenOrg[k][l].r, 0), 255);
            resultado.g += min(max(ppsImagenOrg[k][l].g, 0), 255);
            resultado.b += min(max(ppsImagenOrg[k][l].b * -v, 0), 255);
...
int main(int argc, char **argv)
{
  ...
  Filtro(NUM_PASOS, DIST_RADIO, ImgOrg, ImgDst, n, m);
